...
const list = document.querySelectorAll("[data-name='acc']");    
const iconList = document.querySelectorAll('.elem__iconOp')
const elemIcon = document.querySelectorAll(".section__h2")
const elemActive = document.querySelectorAll(".elem")   


const bla = document.querySelector('.bl1');
// bla.addEventListener('click', blabla);
// function blabla (){
//     elemIcon.classList.toggle('active');
// }


list.forEach(item => {
    item.addEventListener('click', e => {
    if (e.target.classList.contains('elem__details')) {
        e.target.classList.remove('elem__details');
      
    }
    list.nextElementSibling.forEach(e=>e.classList.remove('elem__details'));
    e.target.classList.add('elem__details');
  });
});
...
elemNext.forEach(function(item){
    item.addEventListener("click", closeL)
});

function closeL(){
    bla.classList.toggle('elem__iconCl') 
}
...
// list.forEach(item => {
//     item.addEventListener('click', e => {
//     if (e.target.nextElementSibling.classList.contains('elem__details')) {
//         e.target.nextElementSibling.classList.remove('elem__details');
//       return;
//     }
//     acc.forEach(e=>e.nextElementSibling.classList.remove('elem__details'));
//     e.target.nextElementSibling.classList.add('elem__details');
//   });
// });
//

// list.nextElementSibling.addEventListener('click', listCloseAfter);

// function listCloseAfter() {
